...
class Formbuilder {
	...
	public function generate_html($form_action = false){
		...
		if(is_array($this->_form_array['form_structure'])){
			...
			$html .= '<li class="btn-submit"><input type="submit" name="submit" value="Salvar" /></li>' . "\n";
			$html .=  '</ol>' . "\n";
			$html .=  '</form>' . "\n";
	...
	protected function loadCheckboxGroup($field){
		...
		if(isset($field['values']) && is_array($field['values'])){
			...
			foreach($field['values'] as $item){
				...
				// load post value
				$checked = $item['baseline'];
				
				// if checked, set html
				$checked = $checked === 'true' ? ' checked="checked"' : '';

				$checkbox 	= '<span class="row clearfix"><input type="checkbox" id="%s-%s" name="%s[]" value="%s"%s /><label for="%s-%s">%s</label></span>' . "\n";
				$html .= sprintf($checkbox, $this->elemId($field['title']),$this->elemId($field['title']),  $this->elemId($field['title']), $item['value'], $checked, $this->elemId($field['title']), $this->elemId($item['value']), $item['value']);
			}
			$html .= sprintf('</span>') . "\n";
		}
		$html .= '</li>' . "\n";
	...
	protected function loadSelectBox($field){
		...
		if(isset($field['values']) && is_array($field['values'])){
			...
			foreach($field['values'] as $item){
				...
				// if checked, set html
				$checked = $checked === 'true' ? ' selected="selected"' : '';
	...
	protected function elemId($label, $prepend = false){
		if(is_string($label)){
			...
			return $prepend.strtolower( preg_replace("/[^A-Za-z0-9_]/", "", utf8_encode(strtr(utf8_decode(trim($label)), utf8_decode("áàãâéêíóôõúüçÁÀÃÂÉÊÍÓÔÕÚÜÇ "), "aaaaeeiooouucAAAAEEIOOOUUC_"))));
		}
		return false;
	}
